...
def _edge_realized(ledger: pd.DataFrame) -> float:
    """
    Realized edge = actual win rate minus mean implied probability.
    Positive means model found real edge.
    """
    from pipeline.risk.kelly import american_to_decimal, decimal_to_implied
    try:
...
def _kelly_efficiency(ledger: pd.DataFrame) -> float:
    """
    Mean (actual_return_per_bet / EV_per_bet).
    1.0 = perfect; < 1.0 = model overestimates edge.
    """
    ev = ledger["prob"] * (
        ledger["odds"].apply(
            lambda o: __import__(
                "pipeline.risk.kelly", fromlist=["american_to_decimal"]
            ).american_to_decimal(int(o)) - 1
        )
    ) - (1 - ledger["prob"])
...
def _calibration_table(ledger: pd.DataFrame, n_bins: int = 10) -> pd.DataFrame:
    """
    Reliability diagram data: predicted prob bucket vs actual win rate.
    """
    ledger = ledger.copy()
...
def evaluate(
    ...
    """
    Full evaluation of a backtest ledger.

    Required columns: date, game_pk, side, prob, edge, odds,
                      bet_units, won, pnl, bankroll_pre.

    Returns flat dict of metrics suitable for logging or JSON export.
    """
    if ledger.empty:
...
def calibration_report(ledger: pd.DataFrame, n_bins: int = 10) -> pd.DataFrame:
    """Returns reliability diagram table for plotting."""
    return _calibration_table(ledger, n_bins)


def edge_decay_report(ledger: pd.DataFrame) -> pd.DataFrame:
    """
    Monthly ROI breakdown — reveals if edge degrades over time
    or is concentrated in specific season phases.
    """
    ledger = ledger.copy()
    ledger["date"]  = pd.to_datetime(ledger["date"])
